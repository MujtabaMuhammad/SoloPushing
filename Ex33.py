...
def main():
    ...
    for i in comparative_list:
        if i > 0:
            comparision_list.append("'s salary is below average")
        else:
            comparision_list.append("'s salary is above average")

    for i in range(min(len(employee_names), len(comparision_list))):
        concatenated_list.append(employee_names[i] + comparision_list[i])
